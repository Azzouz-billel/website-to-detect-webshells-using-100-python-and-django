...
class AssetControllerTest extends TestCase
{
    ...
    {
        $module = $this->getMockBuilder('yii\\base\\Module')->setMethods(['fake'])->setConstructorArgs(['console'])->getMock();
    ...
    protected function createCompressConfig(array $bundles, array $config = [])
    {
        ...
        $baseUrl = '/test';
        $config = ArrayHelper::merge($config, ['bundles' => $bundles, 'targets' => [$className => ['basePath' => $this->testAssetsBasePath, 'baseUrl' => $baseUrl, 'js' => 'all.js', 'css' => 'all.css']], 'assetManager' => ['basePath' => $this->testAssetsBasePath, 'baseUrl' => '']]);
        return $config;
    }
    ...
    {
        $config = array_merge(['namespace' => StringHelper::dirname(get_class($this)), 'class' => 'AppAsset', 'sourcePath' => null, 'basePath' => $this->testFilePath, 'baseUrl' => '', 'css' => [], 'js' => [], 'depends' => []], $config);
...
EOL;
    ...
    {
        $cssFiles = ['css/test_body.css' => 'body {
                padding-top: 20px;
                padding-bottom: 60px;
        ...
        $jsFiles = ['js/test_alert.js' => "function test() {\n                alert('Test message');\n            }", 'js/test_sum_ab.js' => 'function sumAB(a, b) {
                return a + b;
            }'];
        $this->createAssetSourceFiles($jsFiles);
        $assetBundleClassName = $this->declareAssetBundleClass(['css' => array_keys($cssFiles), 'js' => array_keys($jsFiles)]);
    ...
    {
        $externalAssetConfig = ['class' => 'ExternalAsset', 'sourcePath' => null, 'basePath' => null, 'js' => ['//ajax.googleapis.com/ajax/libs/jquery/2.1.1/jquery.min.js'], 'css' => ['//ajax.googleapis.com/css/libs/jquery/2.1.1/jquery.ui.min.css']];
        $externalAssetBundleClassName = $this->declareAssetBundleClass($externalAssetConfig);
        $cssFiles = ['css/test.css' => 'body {
                padding-top: 20px;
                padding-bottom: 60px;
            }'];
        $this->createAssetSourceFiles($cssFiles);
        $jsFiles = ['js/test.js' => "function test() {\n                alert('Test message');\n            }"];
        $this->createAssetSourceFiles($jsFiles);
        $regularAssetBundleClassName = $this->declareAssetBundleClass(['class' => 'RegularAsset', 'css' => array_keys($cssFiles), 'js' => array_keys($jsFiles), 'depends' => [$externalAssetBundleClassName]]);
        $bundles = [$regularAssetBundleClassName];
        $bundleFile = $this->testFilePath . DIRECTORY_SEPARATOR . 'bundle.php';
    ...
    public function testDetectCircularDependency()
    {
        ...
        $this->declareAssetBundleClass(['namespace' => $namespace, 'class' => 'AssetStart', 'depends' => [$namespace . '\\AssetA']]);
        $this->declareAssetBundleClass(['namespace' => $namespace, 'class' => 'AssetA', 'depends' => [$namespace . '\\AssetB']]);
        $this->declareAssetBundleClass(['namespace' => $namespace, 'class' => 'AssetB', 'depends' => [$namespace . '\\AssetC']]);
        $this->declareAssetBundleClass(['namespace' => $namespace, 'class' => 'AssetC', 'depends' => [$namespace . '\\AssetA']]);
        $bundles = [$namespace . '\\AssetStart'];
        $bundleFile = $this->testFilePath . DIRECTORY_SEPARATOR . 'bundle.php';
        $configFile = $this->testFilePath . DIRECTORY_SEPARATOR . 'config.php';
        $this->createCompressConfigFile($configFile, $bundles);
    ...
    {
        return [['/linux/absolute/path', '/linux/absolute/path'], ['/linux/up/../path', '/linux/path'], ['/linux/twice/up/../../path', '/linux/path'], ['/linux/../mix/up/../path', '/mix/path'], ['C:\\windows\\absolute\\path', 'C:\\windows\\absolute\\path'], ['C:\\windows\\up\\..\\path', 'C:\\windows\\path']];
    }
    ...
    public function testActionCompressDeleteSource()
    {
        ...
        $assetBundleClassName = $this->declareAssetBundleClass(['class' => 'AssetDelete', 'css' => array_keys($cssFiles), 'js' => array_keys($jsFiles), 'basePath' => null, 'sourcePath' => $sourcePath]);
        $bundles = [$assetBundleClassName];
        $bundleFile = $this->testFilePath . DIRECTORY_SEPARATOR . 'bundle.php';
        $configFile = $this->testFilePath . DIRECTORY_SEPARATOR . 'config_no_source_delete.php';
        $this->createCompressConfigFile($configFile, $bundles, ['deleteSource' => false]);
        $this->runAssetControllerAction('compress', [$configFile, $bundleFile]);
        $files = FileHelper::findFiles($this->testAssetsBasePath, ['only' => ['test_body.css', 'test_alert.js']]);
        $this->assertNotEmpty($files);
        $configFile = $this->testFilePath . DIRECTORY_SEPARATOR . 'config_source_delete.php';
        $this->createCompressConfigFile($configFile, $bundles, ['deleteSource' => true]);
        $this->runAssetControllerAction('compress', [$configFile, $bundleFile]);
        $files = FileHelper::findFiles($this->testAssetsBasePath, ['only' => ['test_body.css', 'test_alert.js']]);
    ...
    {
        $cssFiles = ['css/override_external.css' => 'body {
                padding-top: 20px;
                padding-bottom: 60px;
            }'];
        $this->createAssetSourceFiles($cssFiles);
        $jsFiles = ['js/override_external.js' => "function test() {\n                alert('Test message');\n            }"];
        $assetBundleClassName = $this->declareAssetBundleClass(['class' => 'AssetOverrideExternal', 'css' => array_keys($cssFiles), 'js' => array_keys($jsFiles)]);
        $bundles = [$assetBundleClassName];
        ...
        $assetBundleOverrideConfig = ['sourcePath' => null, 'basePath' => null, 'baseUrl' => null, 'css' => ['//some.cdn.com/js/override_external.css'], 'js' => ['//some.cdn.com/js/override_external.js']];
        $this->createCompressConfigFile($configFile, $bundles, ['assetManager' => ['bundles' => [$assetBundleClassName => $assetBundleOverrideConfig]]]);
        $this->runAssetControllerAction('compress', [$configFile, $bundleFile]);
