...
class PluginManager extends Plugin
{
    ...
    public static $corePlugins = ['Croogo/Acl', 'Croogo/Core', 'Croogo/Extensions', 'Croogo/Install', 'Croogo/Settings', 'Migrations', 'Search'];
    public static $bundledPlugins = ['Croogo/Blocks', 'Croogo/Contacts', 'Croogo/Dashboards', 'Croogo/FileManager', 'Croogo/Meta', 'Croogo/Menus', 'Croogo/Nodes', 'Croogo/Taxonomy', 'Croogo/Users'];
    ...
    public function getPlugins($type = 'plugin')
    {
        ...
        unset($pluginPaths['Croogo']);
        foreach ($pluginPaths as $pluginName => $pluginPath) {
            $this->folder->path = $pluginPath;
            if (!file_exists($this->folder->path)) {
                continue;
            }
            if ($type === 'plugin' && $this->_isCroogoPlugin($pluginPath) || $type === 'theme' && $this->_isCroogoTheme($pluginPath)) {
    ...
    protected function _isCroogoTheme($pluginDir, $path = '')
    {
        ...
        $themeConfigs = ['config' . DS . 'theme.json', 'webroot' . DS . 'theme.json'];
    ...
    protected function _isCroogoPlugin($pluginDir, $path = '')
    {
        ...
        if (file_exists($composerFile) && !$this->_isCroogoTheme($pluginDir, $path)) {
            ...
            if (isset($pluginData['require']['croogo/core']) || isset($pluginData['require']['croogo/croogo'])) {
                return true;
            }
        }
        return false;
    ...
    protected function _loadData($alias, $pluginPath, $ignoreMigration = true)
    {
        ...
        if ($hasManifest || $hasComposer) {
            $pluginData = ['name' => $alias, 'needMigration' => false, 'active' => $active];
            if ($hasManifest) {
                $manifestData = json_decode(file_get_contents($manifestFile), true);
            ...
            if (!$ignoreMigration && $this->needMigration($alias, $active)) {
                $pluginData = ['name' => $alias, 'description' => "Croogo {$alias} plugin", 'active' => true, 'needMigration' => true];
                return $pluginData;
            }
        }
        return false;
    ...
    public function needMigration($plugin, $isActive)
    {
        ...
        $options = ['connection' => static::migrationConnectionName()];
        if ($plugin !== 'app') {
    ...
    public function migrate($plugin)
    {
        ...
        try {
            $migrated = $this->_getMigrations()->migrate($options);
    ...
    public function seed($plugin)
    {
        ...
        return $this->_getMigrations()->seed($options);
    }
    public function unmigrate($plugin)
    {
        ...
        $options = ['connection' => static::migrationConnectionName(), 'target' => 0];
        if ($plugin !== 'app') {
            $options['plugin'] = $plugin;
        }
        return $this->_getMigrations()->rollback($options);
    }
    ...
    public function getActivator($plugin = null)
    {
        ...
        if (!isset($this->_PluginActivation)) {
            ...
            if (isset($pluginPaths[$plugin])) {
                ...
                if (file_exists($configFile) && (include $configFile) || file_exists($activationFile) && (include $activationFile)) {
    ...
    public function activate($plugin, $dependencyList = [])
    {
        ...
        if (!isset($pluginActivation) || isset($pluginActivation) && method_exists($pluginActivation, 'beforeActivation') && $pluginActivation->beforeActivation($this->_Controller)) {
            ...
            if (!empty($missingPlugins)) {
                return __dn('croogo', 'Plugin "%2$s" requires the "%3$s" plugin to be installed.', 'Plugin "%2$s" requires the %3$s plugins to be installed.', count($missingPlugins), $plugin, Text::toList($missingPlugins));
            }
            try {
                static::load($plugin);
    ...
    public static function bootstrap($name)
    {
        ...
        return static::_includeFile($plugin->getConfigPath() . 'bootstrap.php', true);
    }
    public static function load($plugin, array $config = [])
    {
        ...
        $config += ['autoload' => false, 'bootstrap' => false, 'routes' => false, 'console' => true, 'classBase' => 'src', 'ignoreMissing' => false, 'events' => false, 'name' => $plugin];
    ...
    public static function setup(PluginApplicationInterface $app)
    {
        ...
        $cacheConfig = ['duration' => '+1 hour', 'path' => CACHE . 'queries' . DS, 'className' => $defaultEngine, 'prefix' => $defaultPrefix] + $defaultCacheConfig;
        ...
        if (!in_array('cached_settings', $configured)) {
            Cache::setConfig('cached_settings', array_merge(Configure::read('Croogo.Cache.defaultConfig'), ['groups' => ['settings']]));
        }
    ...
    {
        Configure::write('DebugKit.panels', array_merge((array) Configure::read('DebugKit.panels'), ['Croogo/Core.Plugins', 'Croogo/Core.ViewHelpers', 'Croogo/Core.Components']));
        Croogo::hookComponent('*', ['Croogo' => ['className' => 'Croogo/Core.Croogo', 'priority' => 5]]);
        Croogo::hookComponent('*', 'Croogo/Acl.Filter');
        Croogo::hookComponent('*', ['Security' => ['blackHoleCallback' => '_securityError']]);
        ...
        time(function () {
            ...
            $corePlugins = ['Croogo/Settings', 'Croogo/Acl', 'Croogo/Blocks', 'Croogo/Comments', 'Croogo/Contacts', 'Croogo/Menus', 'Croogo/Meta', 'Croogo/Nodes', 'Croogo/Taxonomy', 'Croogo/Users', 'Croogo/Wysiwyg', 'Croogo/Ckeditor', 'Croogo/Dashboards'];
        ...
        time(function () use($app) {
            ...
            $app->addPlugin('Croogo/Extensions', ['autoload' => true, 'bootstrap' => true, 'routes' => true, 'events' => true]);
        ...
        time(function () use($app, $plugins, $themes) {
            $option = ['autoload' => true, 'bootstrap' => true, 'ignoreMissing' => true, 'routes' => true, 'events' => true];
            foreach ($plugins as $plugin) {
                $plugin = Inflector::camelize($plugin);
                if (Plugin::isLoaded($plugin)) {
                    continue;
            ...
            foreach ($themes as $theme) {
                if ($theme && !Plugin::isLoaded($theme) && PluginManager::available($theme)) {
                    PluginManager::load($theme, ['autoload' => true, 'bootstrap' => true, 'routes' => true, 'events' => true, 'ignoreMissing' => true]);
                }
            }
